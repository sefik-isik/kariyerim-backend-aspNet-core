...
namespace Business.Concrete
{
    public class CompanyUserAdvertJobDescriptionManager : ICompanyUserAdvertJobDescriptionService
    {
        ...
        public IDataResult<List<CompanyUserAdvertJobDescriptionDTO>> GetAllDTO(UserAdminDTO userAdminDTO)
        {
            ...
            if (userIsAdmin.Data == null)
            {
                return new SuccessDataResult<List<CompanyUserAdvertJobDescriptionDTO>>(_companyUserAdvertJobDescriptionDal.GetAllDTO().FindAll(c => c.Id == userAdminDTO.Id && c.UserId == userAdminDTO.UserId).OrderBy(s => s.AdvertId).ToList());
            }
            else
            {
        ...
        public IDataResult<List<CompanyUserAdvertJobDescriptionDTO>> GetDeletedAllDTO(UserAdminDTO userAdminDTO)
        {
            ...
            if (userIsAdmin.Data == null)
            {
                return new SuccessDataResult<List<CompanyUserAdvertJobDescriptionDTO>>(_companyUserAdvertJobDescriptionDal.GetDeletedAllDTO().FindAll(c => c.Id == userAdminDTO.Id && c.UserId == userAdminDTO.UserId).OrderBy(s => s.AdvertId).ToList());
            }
            else
            {
