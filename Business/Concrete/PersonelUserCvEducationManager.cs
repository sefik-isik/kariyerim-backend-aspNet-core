...
namespace Business.Concrete
{
    public class PersonelUserCvEducationManager : IPersonelUserCvEducationService
    {
        ...
        public async Task<IDataResult<List<PersonelUserCvEducationDTO>>> GetAllDTO(UserAdminDTO userAdminDTO)
        {
            ...
            if (userIsAdmin.Data == null)
            {
                return new SuccessDataResult<List<PersonelUserCvEducationDTO>>(alldto.OrderBy(x => x.FirstName).OrderBy(x => x.LastName).ToList().FindAll(c => c.UserId == userAdminDTO.UserId).FindAll(c => c.CvId == userAdminDTO.Id), Messages.SuccessListed);
            }
            else
            {
